...
2. Encryption at rest, split in two:
   ...
   - **Files — deliberately not encrypted (project owner's decision, 09Aug2026).** `FILE_ENCRYPTION_KEY` stays a placeholder on purpose. `artifact_service.py`, `envelope_service.py`, and scanned images on disk stay plaintext: these files sit on the project owner's own machine (bind-mounted, not sealed in Docker) under the same threat model as their existing hardcopy/PDF letter archive, and encrypting them would make routine use (open, review, print) meaningfully more cumbersome for a cost judged not worth it. **Agreed mitigation: a warning, not encryption** — see item 2b below.
