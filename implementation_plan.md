...
- **Status:** ⚠️ Partially functional (updated 09Aug2026). `MAPPING_STORE_KEY` is now real and in active use — Postgres's `Prisoner` PII columns are genuinely encrypted at rest (verified via raw SQL). `FILE_ENCRYPTION_KEY` is still a placeholder: scanned images, the saved `data/active_map.xlsx` roster file, and generated PDFs on disk are still plaintext. Same AES-GCM code, not yet wired to the file-writing services. See punch list item 2.
